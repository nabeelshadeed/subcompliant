...
export default function BillingClient({ currentSubs = 0, subLimit = 10, plan = 'starter' }: { currentSubs?: number; subLimit?: number; plan?: string }) {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="card p-5">
        ...
        <div className="flex items-center justify-between text-xs text-white/60 mb-1.5">
          ...
          <span className={cn('font-medium', currentSubs >= subLimit ? 'text-red-400' : currentSubs / subLimit >= 0.8 ? 'text-amber-400' : 'text-white/80')}>
            {currentSubs} / {subLimit}
          </span>
        </div>
        <div className="h-2 bg-white/10 rounded-full overflow-hidden">
          <div
            className={cn(
              ...
              currentSubs >= subLimit ? 'bg-red-400' : currentSubs / subLimit >= 0.8 ? 'bg-amber-400' : 'bg-accent'
            )}
            style={{ width: `${Math.min(100, Math.round((currentSubs / subLimit) * 100))}%` }}
          />
        </div>
        {currentSubs >= subLimit && (
          <p className="text-xs text-red-400 mt-2">You&apos;ve reached your subcontractor limit. Upgrade to add more.</p>
        )}
        {currentSubs / subLimit >= 0.8 && currentSubs < subLimit && (
          <p className="text-xs text-amber-400 mt-2">You&apos;re approaching your limit. Consider upgrading soon.</p>
        )}
      </div>
